...
class InstagrapiClient:
    ...
    def totp_code(self, seed: str) -> str:
        """Derive the current authenticator code locally, with no network call."""
        try:
            return self._client.totp_generate_code(seed.strip().replace(" ", ""))
        except Exception as error:
            raise InstagramRejected(
                "The stored authenticator seed is not a valid base32 setup key"
            ) from error
